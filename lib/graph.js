...
function createGraph(modules, circular, config, options) {
	...
	let allSafeModules=[...config.safeModules,config.baseModule]
	function isExternalModule(id){
		return !allSafeModules.includes(id.split('/')[0])
	}
	...
	let allDependents=Object.values(modules).flatMap(deps=>deps);

	Object.keys(modules).forEach((id) => {
		nodes[id] = nodes[id] || g.addNode(id);
		if(!allDependents.includes(id)){
			nodes[id].set('root',true)
		}
		let externalModule = isExternalModule(id)
		...
		modules[id].forEach((depId) => {
			let node= nodes[depId] = nodes[depId] || g.addNode(depId);

			if (!modules[depId]) {
				setNodeColor(nodes[depId], config.noDependencyColor);
			}

			let externalDependency = isExternalModule(depId)


			if (externalDependency) {
				if (externalModule) {
					if(node.get('group') !== 'crossModule'){
						setNodeColor(nodes[depId], config.externalNestedDependencyColor);
					}
				} else {
					node.set('group',"crossModule")
					setNodeColor(node, config.externalModuleColor);
				}
			}
